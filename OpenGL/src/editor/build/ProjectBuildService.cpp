...
ProjectBuildResult ProjectBuildService::ExecuteBuild(const bool CompleteProject) const
{
	...
	if (CompleteProject)
	{
		...
		const std::wstring Targets =
			HasGameModule ? L"Engine;EngineTools;Editor;Game;Validation;GameModule" : L"Engine;EngineTools;Editor;Game;Validation";
		...
		const std::filesystem::path Validation = BuildDirectory / "Validation.exe";
		const std::filesystem::path Editor = BuildDirectory / "Editor.exe";
		const std::filesystem::path EngineToolsLibrary = BuildDirectory / "EngineTools.lib";
		const std::filesystem::path Game = BuildDirectory / "Game.exe";
		...
		if (!std::filesystem::is_regular_file(EngineToolsLibrary) || !std::filesystem::is_regular_file(Editor) ||
			...
			throw ProjectBuildException(
				HasGameModule ? "Complete project build did not produce EngineTools, Editor, Game, Validation, and GameModule outputs"
							  : "Complete project build did not produce EngineTools, Editor, Game, and Validation outputs");
		const std::filesystem::path EngineLibrary = BuildDirectory / "Engine.dll";
		if (!std::filesystem::is_regular_file(EngineLibrary))
			throw ProjectBuildException("Complete project build did not produce Engine.dll");
