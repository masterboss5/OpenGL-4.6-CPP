...
void RegisterCoreEditorActions(EditorActionRegistry &Registry)
{
	...
	Registry.Register({.ID = IDs::PackageProject,
					   .Name = "Build.Package",
					   .DisplayName = "Package",
					   .Description = "Build a runnable package with explicitly configured runtime files",
					   .Icon = "Package",
					   .Category = EditorActionCategory::Build,
					   .Order = 20,
					   .CanExecute =
					   ...
					   {
						   const cook::CookPackageState State = Context.Session.GetCookPackageService().GetState();
						   return Context.Session.HasRuntimePackageFiles() &&
								  Context.Session.GetProjectBuildService().GetState() != build::ProjectBuildState::Building &&
								  State != cook::CookPackageState::Cooking && State != cook::CookPackageState::Publishing;
					   },
