...
layout(location = 5) out float occlusionOutput;
...
void main()
{
	...
	if (material.baseColorTexture != uint64_t(0))
		albedo *= texture(sampler2D(material.baseColorTexture), materialTextureCoordinate(material, 0u));
	if ((inputData.instanceFlags & 32U) != 0U && albedo.a < material.roughnessTransmissionIor.w)
		discard;
	if (trackOverdraw != 0U)
	...
	bool velocityValid = inputData.currentClip.w > 0.00001 && inputData.previousClip.w > 0.00001;
	vec2 currentNdc = velocityValid ? inputData.currentClip.xy / inputData.currentClip.w : vec2(0.0);
	vec2 previousNdc = velocityValid ? inputData.previousClip.xy / inputData.previousClip.w : currentNdc;
	...
	if ((inputData.instanceFlags & 16U) == 0U)
		storedMetallic = -storedMetallic - 1.0;
