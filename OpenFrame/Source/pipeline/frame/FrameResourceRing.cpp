...
FrameBufferSlice FrameResourceRing::CreateBuffer(uint64 CapacityInBytes)
{
	...
	if (CapacityInBytes > static_cast<uint64>(std::numeric_limits<GLsizeiptr>::max()))
		throw std::overflow_error("Frame resource capacity exceeds OpenGL buffer-size limits");
	FrameBufferSlice Slice{.CapacityInBytes = CapacityInBytes};
	glCreateBuffers(1, &Slice.Buffer);
	if (Slice.Buffer == 0)
		throw pipeline::device::DeviceError("OpenGL could not allocate a frame-resource buffer");
	try
	{
		...
		this->Device->CheckErrors("Frame resource buffer allocation");
		return Slice;
	}
	catch (...)
	{
	...
	: Device(Device)
{
	...
	try
	{
		for (Slot &Slot : Slots)
		{
			...
			Slot.Resources.SelectionMask = CreateBuffer(Capacities.SelectionMask);
			Slot.Resources.DebugLines = CreateBuffer(Capacities.DebugLines);
		}
...
FrameResourceRing::~FrameResourceRing()
{
	...
	const bool CanReleaseGPUResources = LiveDevice != nullptr && LiveDevice->CanIssueCommands();
	for (Slot &Slot : Slots)
	{
		...
		if (Slot.Fence.IsValid())
		{
			try
			{
				LiveDevice->DestroySync(Slot.Fence);
			}
			catch (...)
			{
				Slot.Fence = {};
...
void FrameResourceRing::DestroyBuffer(FrameBufferSlice &Slice) noexcept
{
	if (Slice.Buffer != 0)
	{
		if (Slice.MappedMemory != nullptr)
			glUnmapNamedBuffer(Slice.Buffer);
		if (pipeline::device::Device *LiveDevice = this->Device.TryGet(); LiveDevice != nullptr)
			LiveDevice->RetireGPUObject(pipeline::device::GPUObjectType::Buffer, Slice.Buffer);
	}
	Slice = {};
}
...
void FrameResourceRing::DestroyAllBuffers() noexcept
{
	for (Slot &Slot : this->Slots)
	{
		...
		DestroyBuffer(Slot.Resources.SelectionMask);
		DestroyBuffer(Slot.Resources.DebugLines);
	}
}
...
FrameResources &FrameResourceRing::Acquire(uint64 FrameNumber)
{
	...
	if (Slot.Resources.FrameNumber != 0)
		this->Device->CollectRetiredGPUObjects(Slot.Resources.FrameNumber);
	Slot.AssetPins.clear();
...
void FrameResourceRing::Retire(std::vector<resource::AssetPtr<resource::Asset>> &&AssetPins)
{
	...
	Slot.Fence = Fence;
	this->Device->NotifyFrameSubmitted(Slot.Resources.FrameNumber);
	ActiveSlot = ~uint32{0};
}
