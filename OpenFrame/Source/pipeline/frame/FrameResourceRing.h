...
struct FrameResources final
{
	...
	FrameBufferSlice SelectionMask;
	FrameBufferSlice DebugLines;
...
struct FrameResourceCapacitySpecification final
{
	...
	uint64 SelectionMask = 0;
	uint64 DebugLines = 0;
};

class ENGINE_API FrameResourceRing final
{
  public:
	...
	// Swaps reusable pin storage with the retired slot to avoid per-frame reallocations.
	void Retire(std::vector<resource::AssetPtr<resource::Asset>> &AssetPins);
  ...
  private:
	...
	void DestroyBuffer(FrameBufferSlice &Slice) noexcept;
