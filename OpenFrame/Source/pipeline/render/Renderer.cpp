...
constexpr uint32 HeadlessTemporalConvergenceFrames = 16U;
...
constexpr float32 MaximumDirectionalShadowDistance = 500.0f;
...
[[nodiscard]] float32 RadicalInverse(uint32 Index, const uint32 Base) noexcept
{
	float32 Result = 0.0f;
...
[[nodiscard]] glm::vec2 CalculateTemporalJitter(const uint64 ViewFrameNumber, const core::WindowExtent Extent) noexcept
{
	constexpr uint32 SequenceLength = 16U;
	const uint32 SampleIndex = static_cast<uint32>(ViewFrameNumber % SequenceLength) + 1U;
	const glm::vec2 CenteredSample{RadicalInverse(SampleIndex, 2U) - 0.5f, RadicalInverse(SampleIndex, 3U) - 0.5f};
	return CenteredSample * glm::vec2(2.0f / static_cast<float32>(Extent.Width), 2.0f / static_cast<float32>(Extent.Height));
}

[[nodiscard]] glm::mat4 ApplyProjectionJitter(glm::mat4 Projection, const glm::vec2 Jitter) noexcept
{
	// Add the sub-pixel offset in homogeneous clip space. Adding a multiple of
	// the W row works for both perspective and orthographic projections.
	for (uint32 Column = 0U; Column < 4U; ++Column)
	{
		Projection[Column][0] += Jitter.x * Projection[Column][3];
		Projection[Column][1] += Jitter.y * Projection[Column][3];
	}
	return Projection;
}

[[nodiscard]] std::array<glm::vec3, 8> BuildCameraFrustumSliceCorners(const Camera &Camera, const core::WindowExtent Extent,
																	  const float32 SliceNear, const float32 SliceFar)
{
	if (!Extent.IsValid() || !std::isfinite(SliceNear) || !std::isfinite(SliceFar) || SliceNear <= 0.0f || SliceFar <= SliceNear)
		throw std::invalid_argument("Directional shadow cascade requires a finite positive camera-frustum slice");
	...
	if (!IsFinite(Camera.Position) || !IsFinite(Forward) || !IsFinite(Right) || !IsFinite(Up))
		throw std::invalid_argument("Directional shadow cascade requires a finite camera basis");

	float32 NearHalfHeight = Camera.OrthographicHeight * 0.5f;
	float32 FarHalfHeight = NearHalfHeight;
	if (Camera.Projection == CameraProjectionMode::Perspective)
	{
		const float32 Tangent = std::tan(glm::radians(Camera.FOV) * 0.5f);
		if (!std::isfinite(Tangent) || Tangent <= 0.0f)
			throw std::invalid_argument("Directional shadow cascade requires a valid camera field of view");
		NearHalfHeight = SliceNear * Tangent;
		FarHalfHeight = SliceFar * Tangent;
	}
	if (!std::isfinite(NearHalfHeight) || !std::isfinite(FarHalfHeight) || NearHalfHeight <= 0.0f || FarHalfHeight <= 0.0f)
		throw std::invalid_argument("Directional shadow cascade requires a valid camera projection extent");
	const float32 Aspect = Extent.AspectRatio();
	const float32 NearHalfWidth = NearHalfHeight * Aspect;
	const float32 FarHalfWidth = FarHalfHeight * Aspect;
	const glm::vec3 NearCenter = Camera.Position + Forward * SliceNear;
	const glm::vec3 FarCenter = Camera.Position + Forward * SliceFar;
	return {NearCenter - Right * NearHalfWidth - Up * NearHalfHeight, NearCenter + Right * NearHalfWidth - Up * NearHalfHeight,
			NearCenter + Right * NearHalfWidth + Up * NearHalfHeight, NearCenter - Right * NearHalfWidth + Up * NearHalfHeight,
			FarCenter - Right * FarHalfWidth - Up * FarHalfHeight,	  FarCenter + Right * FarHalfWidth - Up * FarHalfHeight,
			FarCenter + Right * FarHalfWidth + Up * FarHalfHeight,	  FarCenter - Right * FarHalfWidth + Up * FarHalfHeight};
}

[[nodiscard]] glm::mat4 BuildStableDirectionalShadowMatrix(const Camera &Camera, const core::WindowExtent Extent,
														   const glm::vec3 &Direction, const float32 SliceNear, const float32 SliceFar,
														   const uint32 ShadowResolution)
{
	if (!IsFinite(Direction) || glm::dot(Direction, Direction) <= 1.0e-8f || ShadowResolution == 0U)
		throw std::invalid_argument("Directional shadow cascade requires a finite light direction and nonzero resolution");
	const glm::vec3 LightDirection = glm::normalize(Direction);
	const std::array<glm::vec3, 8> Corners = BuildCameraFrustumSliceCorners(Camera, Extent, SliceNear, SliceFar);
	glm::vec3 Center(0.0f);
	for (const glm::vec3 &Corner : Corners)
		Center += Corner;
	Center /= static_cast<float32>(Corners.size());
	const glm::vec3 LightUp = glm::abs(LightDirection.y) > 0.95f ? glm::vec3(1.0f, 0.0f, 0.0f) : glm::vec3(0.0f, 1.0f, 0.0f);
	const float32 DepthPadding = std::max(50.0f, SliceFar - SliceNear);
	const glm::mat4 View = glm::lookAt(Center - LightDirection * (MaximumDirectionalShadowDistance + DepthPadding), Center, LightUp);
	...
	if (!IsFinite(Minimum) || !IsFinite(Maximum))
		throw std::runtime_error("Directional shadow cascade produced non-finite light-space bounds");
	const glm::vec2 HalfExtent = glm::max((glm::vec2(Maximum) - glm::vec2(Minimum)) * 0.5f, glm::vec2(0.001f));
	const glm::vec2 TexelSize = (HalfExtent * 2.0f) / static_cast<float32>(ShadowResolution);
	const glm::vec2 PaddedHalfExtent = HalfExtent + TexelSize * 2.0f;
	glm::vec2 BoundsCenter = (glm::vec2(Minimum) + glm::vec2(Maximum)) * 0.5f;
	BoundsCenter = glm::round(BoundsCenter / TexelSize) * TexelSize;
	const float32 NearPlane = std::max(0.1f, -Maximum.z - DepthPadding);
	const float32 FarPlane = std::max(NearPlane + 1.0f, -Minimum.z + DepthPadding);
	const glm::mat4 Projection =
		glm::orthoRH_ZO(BoundsCenter.x - PaddedHalfExtent.x, BoundsCenter.x + PaddedHalfExtent.x, BoundsCenter.y - PaddedHalfExtent.y,
						BoundsCenter.y + PaddedHalfExtent.y, NearPlane, FarPlane);
	const glm::mat4 Result = Projection * View;
	if (!IsFinite(Result))
		throw std::runtime_error("Directional shadow cascade produced a non-finite view-projection matrix");
	return Result;
}
					  ...
					  const std::span<const pipeline::render::ViewportPickPixel> PickPixels)
{
	...
	if (State.PreviousRenderTransforms.bucket_count() == 0)
	{
		...
		State.PreviousLODSelections.reserve(MaximumRenderItems);
		State.CurrentLODSelections.reserve(MaximumRenderItems);
	}
	...
	if (State.RenderTransformGeneration == 0)
	{
		...
		State.PreviousLODSelections.clear();
		State.CurrentLODSelections.clear();
	...
	pipeline::render::SceneGpuResolver Extractor(this->Device.Get(), this->MeshGPUCache, Assets, State.PreviousRenderTransforms,
												 ...
												 State.PreviousLODSelections, State.CurrentLODSelections, State.RenderTransformGeneration,
												 this->SceneExtractorScratchStorage);
	...
	State.PreviousLODSelections.swap(State.CurrentLODSelections);
	...
	if (State.PreviousLODSelections.size() > static_cast<usize>(MaximumRenderItems) * 2U)
	{
		const uint64 PublishedGeneration = State.PreviousRenderTransformGeneration;
		std::erase_if(State.PreviousLODSelections,
					  [PublishedGeneration](const auto &Entry) { return Entry.second.Generation != PublishedGeneration; });
	}
									...
									ViewState &State)
{
	...
	const glm::mat4 UnjitteredProjection = Camera.GetProjectionMatrix(Extent);
	const glm::vec2 TemporalJitter = Camera.TemporalJitterEnabled ? CalculateTemporalJitter(State.FrameNumber, Extent) : glm::vec2(0.0f);
	const glm::mat4 Projection = ApplyProjectionJitter(UnjitteredProjection, TemporalJitter);
											  ...
											  const core::WindowExtent WindowExtent, const RenderViewID ViewID)
{
	...
	const bool JitterModeChanged = View.HasPreviousCameraState && View.PreviousTemporalJitterEnabled != Camera.TemporalJitterEnabled;
	const bool CameraCut =
		View.HasPreviousCameraState &&
		(glm::distance(Camera.Position, View.PreviousCameraPosition) > 2.0f ||
		 ...
		 glm::dot(glm::normalize(Camera.Up), glm::normalize(View.PreviousCameraUp)) < 0.999f || ProjectionChanged || JitterModeChanged);
	const glm::vec3 CameraTranslation = Camera.Position - View.PreviousCameraPosition;
	const bool CameraMoved = View.HasPreviousCameraState &&
							 (glm::dot(CameraTranslation, CameraTranslation) > 1.0e-8f ||
							  glm::dot(glm::normalize(Camera.Front), glm::normalize(View.PreviousCameraFront)) < 0.999999f ||
							  glm::dot(glm::normalize(Camera.Up), glm::normalize(View.PreviousCameraUp)) < 0.999999f || ProjectionChanged);
	// Previous-frame occlusion is not conservative across camera motion: an
	// object hidden in the previous view may become exposed in the current
	// view. Preserve temporal color history for smooth movement, but bypass
	// Hi-Z until a stationary frame has established history for the same view.
	if (CameraMoved)
		View.HierarchicalDepthHistoryValid = false;
	if (CameraCut)
	{
	...
	uint32 EffectiveDirectionalShadowResolution = pipeline::render::DirectionalShadowResolution;
	...
	if (ShadowDirectionalLight != nullptr)
	{
		const LightShadowParameters &ShadowParameters = ShadowDirectionalLight->ShadowParameters;
		EffectiveDirectionalShadowResolution = ShadowParameters.Resolution;
		if (EffectiveDirectionalShadowResolution < pipeline::render::MinimumDirectionalShadowResolution ||
			EffectiveDirectionalShadowResolution > pipeline::render::MaximumDirectionalShadowResolution ||
			(EffectiveDirectionalShadowResolution & (EffectiveDirectionalShadowResolution - 1U)) != 0U)
			throw std::invalid_argument("Directional light shadow resolution must be a power of two from 256 through 8192");
		DirectionalCascadeCount = ShadowDirectionalLight->CascadeCount;
		if (DirectionalCascadeCount == 0U || DirectionalCascadeCount > pipeline::render::DirectionalShadowCascadeCount)
			throw std::invalid_argument("Directional light cascade count exceeds the renderer cascade budget");
		const float32 DistributionExponent = ShadowDirectionalLight->CascadeDistributionExponent;
		if (!std::isfinite(DistributionExponent) || DistributionExponent <= 0.0f)
			throw std::invalid_argument("Directional light cascade distribution exponent must be finite and positive");
		if (!std::isfinite(Camera.NearPlane) || !std::isfinite(Camera.FarPlane) || Camera.NearPlane <= 0.0f ||
			Camera.FarPlane <= Camera.NearPlane)
			throw std::invalid_argument("Directional shadow cascades require valid camera clip planes");
		const float32 ShadowFar = std::min(Camera.FarPlane, MaximumDirectionalShadowDistance);
		if (ShadowFar <= Camera.NearPlane)
			throw std::invalid_argument("Directional shadow distance does not contain a valid camera-frustum slice");
		// Distribute cascades by a constant distance ratio. The former polynomial
		// sequence jumped from roughly 8 to 63 world units at the default setting,
		// so an object only ten units from the camera was rendered by a drastically
		// lower-density second cascade. The authored exponent now controls the
		// logarithmic near anchor: the default produces approximately
		// 16 / 50 / 158 / 500 unit split distances.
		const float32 DistributionAnchor = std::max(Camera.NearPlane, ShadowFar / std::pow(10.0f, DistributionExponent));
		const float32 DistributionRatio = ShadowFar / DistributionAnchor;
		const glm::vec3 Direction = glm::normalize(ShadowDirectionalLight->Direction);
		float32 SplitNear = Camera.NearPlane;
		for (uint32 Cascade = 0; Cascade < DirectionalCascadeCount; ++Cascade)
		{
			const float32 NormalizedSplit = static_cast<float32>(Cascade + 1U) / static_cast<float32>(DirectionalCascadeCount);
			const float32 SplitFar =
				Cascade + 1U == DirectionalCascadeCount ? ShadowFar : DistributionAnchor * std::pow(DistributionRatio, NormalizedSplit);
			ShadowRecords[Cascade] = {.ViewProjection = BuildStableDirectionalShadowMatrix(Camera, WindowExtent, Direction, SplitNear,
																						   SplitFar, EffectiveDirectionalShadowResolution),
									  .AtlasScaleBias = glm::vec4(1.0f, 1.0f, static_cast<float32>(Cascade), 0.0f),
									  .DepthBiasAndFilter = glm::vec4(ShadowParameters.ConstantBias, ShadowParameters.SlopeBias,
																	  ShadowParameters.NormalBias, ShadowParameters.FilterRadius),
									  .CascadeData = glm::vec4(SplitNear, SplitFar, static_cast<float32>(DirectionalCascadeCount),
															   static_cast<float32>(EffectiveDirectionalShadowResolution))};
			SplitNear = SplitFar;
		}
	}
	...
	for (const SpotLightSource &Light : SpotLights)
	{
		...
		ShadowRecords[pipeline::render::DirectionalShadowCascadeCount + SpotIndex] = {
			...
			.DepthBiasAndFilter = glm::vec4(Light.ShadowParameters.ConstantBias, Light.ShadowParameters.SlopeBias,
											Light.ShadowParameters.NormalBias, Light.ShadowParameters.FilterRadius),
			.CascadeData = glm::vec4(0.1f, Range, 0.0f, static_cast<float32>(Light.ShadowParameters.Resolution))};
	}
	...
	for (const PointLightSource &Light : PointLights)
	{
		...
		for (uint32 Face = 0; Face < 6; ++Face)
		{
			...
			ShadowRecords[RecordIndex] = {
				.ViewProjection = glm::perspectiveRH_ZO(glm::radians(90.0f), 1.0f, 0.1f, Range) *
	...
	const pipeline::graph::HybridDeferredFrameInputs Inputs{.Extent = Extent,
															...
															.DirectionalShadowResolution = EffectiveDirectionalShadowResolution,
	...
	View.PreviousTemporalJitterEnabled = Camera.TemporalJitterEnabled;
	...
	if (this->HeadlessPresentationValidation && this->HeadlessValidationFrameEligible &&
		this->HeadlessEligibleFrameCount < HeadlessTemporalConvergenceFrames)
		++this->HeadlessEligibleFrameCount;
	++View.FrameNumber;
	++FrameNumber;
...
{
	if (this->PresentationValidated || this->HeadlessEligibleFrameCount + 1U < HeadlessTemporalConvergenceFrames)
		return;
