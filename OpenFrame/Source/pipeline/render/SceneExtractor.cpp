...
void SceneExtractor::Extract(const SceneRenderSnapshot &Snapshot, const Camera &Camera, SceneCollection &Output) const
{
	...
	for (const SceneMeshSnapshot &MeshSnapshot : Snapshot.Meshes)
	{
		...
		for (const resource::ModelMeshInstance &Instance : Model->GetMeshInstances())
		{
			...
			std::optional<uint32> PreviousLOD;
			const auto PreviousLODSelection = this->PreviousLODs->find(HistoryKey);
			if (PreviousLODSelection != this->PreviousLODs->end() && PreviousLODSelection->second.Generation == this->PreviousGeneration)
				PreviousLOD = PreviousLODSelection->second.Level;
			uint32 RequestedLOD = this->SelectLOD(*Mesh, MeshSnapshot.LODPolicy, NodeTransforms[Instance.NodeIndex], Camera, PreviousLOD);
			// Built-in curved primitives are shared editor assets whose shadows are
			// frequently inspected at a much larger projected size than the caster
			// itself. Camera-distance LOD selection can therefore turn an off-screen
			// sphere or cone into a visibly polygonal shadow. Keep the source
			// silhouette for these inexpensive shared shadow casters; imported assets
			// retain their authored LOD policy.
			if (HasFlag(MeshSnapshot.Visibility, components::MeshVisibilityFlags::CastsShadows) &&
				Mesh->GetDerivedDataKey().starts_with("PrimitiveMesh:"))
				RequestedLOD = 0U;
			this->CurrentLODs->insert_or_assign(HistoryKey,
												RenderLODHistoryEntry{.Level = RequestedLOD, .Generation = this->CurrentGeneration});
			const MeshGPULOD *GPULOD = this->MeshCache->TryGetLOD(Mesh, RequestedLOD, Output.GetFrameNumber());
			if (GPULOD == nullptr)
				continue;
			const uint32 LODIndex = GPULOD->Level;
...
uint32 SceneExtractor::SelectLOD(const resource::MeshAsset &Mesh, const components::MeshLODPolicy &Policy, const glm::mat4 &WorldTransform,
								 const Camera &Camera, const std::optional<uint32> PreviousLOD) const
{
	...
	if (PreviousLOD.has_value() && *PreviousLOD < LODs.size() && *PreviousLOD != Selected)
	{
		const uint32 Previous = *PreviousLOD;
		if (Selected > Previous)
		{
			const float32 Boundary = LODs[Previous].ScreenCoverage;
			const float32 Margin = LODs[Previous].Hysteresis;
			if (Coverage > std::max(0.0f, Boundary - Margin))
				Selected = Previous;
		}
		else
		{
			const uint32 BoundaryLOD = Previous - 1U;
			const float32 Boundary = LODs[BoundaryLOD].ScreenCoverage;
			const float32 Margin = LODs[BoundaryLOD].Hysteresis;
			if (Coverage < std::min(1.0f, Boundary + Margin))
				Selected = Previous;
		}
	}
	return Selected;
