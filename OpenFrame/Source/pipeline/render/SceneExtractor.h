...
struct RenderLODHistoryEntry final
{
	uint32 Level = 0;
	uint64 Generation = 0;
};

using RenderLODHistory = std::unordered_map<RenderTransformHistoryKey, RenderLODHistoryEntry, RenderTransformHistoryKeyHash>;
...
class ENGINE_API SceneExtractor final
{
  public:
	SceneExtractor(pipeline::device::Device &Device, pipeline::mesh::MeshGPUCache &MeshCache, resource::AssetManager &Assets,
				   ...
				   const RenderLODHistory &PreviousLODs, RenderLODHistory &CurrentLODs, uint64 CurrentGeneration,
				   SceneExtractorScratch &Scratch)
		: Device(Device), MeshCache(&MeshCache), Assets(&Assets), PreviousTransforms(&PreviousTransforms),
		  ...
		  PreviousLODs(&PreviousLODs), CurrentLODs(&CurrentLODs), Scratch(&Scratch)
	{
	}
  ...
  private:
	...
	const RenderLODHistory *PreviousLODs = nullptr;
	RenderLODHistory *CurrentLODs = nullptr;
	...
	[[nodiscard]] uint32 SelectLOD(const resource::MeshAsset &Mesh, const components::MeshLODPolicy &Policy,
								   const glm::mat4 &WorldTransform, const Camera &Camera, std::optional<uint32> PreviousLOD) const;
