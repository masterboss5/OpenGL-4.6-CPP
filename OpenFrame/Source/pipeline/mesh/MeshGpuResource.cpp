...
const MeshGPULOD *MeshGPUResource::FindNearestResidentLOD(const uint32 Level, const uint64 FrameNumber)
{
	if (Level >= this->LODs.size())
		throw std::out_of_range("Requested mesh GPU LOD is out of range");
	const uint32 LODCount = static_cast<uint32>(this->LODs.size());
	for (uint32 Distance = 1U; Distance < LODCount; ++Distance)
	{
		if (Level >= Distance && this->LODs[Level - Distance] != nullptr)
			return &this->GetLOD(Level - Distance, FrameNumber);
		if (Level + Distance < LODCount && this->LODs[Level + Distance] != nullptr)
			return &this->GetLOD(Level + Distance, FrameNumber);
	}
	return nullptr;
}
...
const MeshGPULOD *MeshGPUCache::TryGetLOD(const resource::AssetPtr<resource::MeshAsset> &Mesh, const uint32 Level, const uint64 FrameNumber)
{
	...
	MeshGPUResource &Resource = this->Realize(Mesh, FrameNumber);
	if (!Resource.IsLODResident(Level))
	{
		this->QueueLOD(Mesh, Level);
		return Resource.FindNearestResidentLOD(Level, FrameNumber);
	}
	return &Resource.GetLOD(Level, FrameNumber);
}
