...
template <typename LightType> void ValidateShadowParameters(const LightType &Light)
{
	const LightShadowParameters &Parameters = Light.ShadowParameters;
	constexpr std::array<uint32, 6> SupportedResolutions{256U, 512U, 1'024U, 2'048U, 4'096U, 8'192U};
	if (std::ranges::find(SupportedResolutions, Parameters.Resolution) == SupportedResolutions.end())
		throw std::invalid_argument("Light shadow resolution is unsupported");
	if (!std::isfinite(Parameters.ConstantBias) || Parameters.ConstantBias < 0.0f || !std::isfinite(Parameters.SlopeBias) ||
		Parameters.SlopeBias < 0.0f || !std::isfinite(Parameters.NormalBias) || Parameters.NormalBias < 0.0f ||
		!std::isfinite(Parameters.FilterRadius) || Parameters.FilterRadius < 0.0f)
	{
		throw std::invalid_argument("Light shadow bias and filter parameters must be finite and non-negative");
	}
}
...
template <IsAttenuatedLightSource LightType> void ValidateAttenuatedLight(const LightType &Light)
{
	...
	ValidateShadowParameters(Light);
											 ...
											 std::vector<pipeline::render::GPULightRecord> &Records) const
{
	...
	for (const DirectionalLightSource &Light : DirectionalLights)
	{
		...
		ValidateShadowFlag(Light.CastShadows);
		ValidateShadowParameters(Light);
		if (!std::isfinite(Light.AngularDiameterDegrees) || Light.AngularDiameterDegrees < 0.0f || Light.AngularDiameterDegrees >= 180.0f)
			throw std::invalid_argument("Directional light angular diameter must be finite and in [0, 180) degrees");
		if (Light.CascadeCount == 0U || Light.CascadeCount > 8U)
			throw std::invalid_argument("Directional light cascade count must be between one and eight");
		if (!std::isfinite(Light.CascadeDistributionExponent) || Light.CascadeDistributionExponent <= 0.0f)
