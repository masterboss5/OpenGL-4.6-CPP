...
#include "Source/pipeline/graph/HybridDeferredFrameGraph.h"
#include "Source/pipeline/graph/RenderPass.h"
#include "Source/pipeline/render/RenderData.h"
#include "Source/pipeline/render/SceneExtractor.h"
#include "Source/pipeline/render/ScenePreparation.h"
#include "Source/pipeline/render/SceneRenderSnapshot.h"
#include "Source/pipeline/render/SelectionMask.h"
#include "Source/pipeline/render/ViewportPicker.h"
...
#include "Source/pipeline/lighting/LightBufferManager.h"
#include "Source/pipeline/mesh/MeshGpuResource.h"
...
#include "Source/pipeline/shader/ShaderException.h"
...
#include "Source/resource/asset/AnimationAsset.h"
...
#include "Source/scene/SceneCommandBuffer.h"
#include "Source/scene/TransformMath.h"
#include "Source/scene/detail/DenseGenerationalPool.h"
#include "Source/util/memory/TypedPoolAllocator.h"
...
void ValidateMaximumShadowBudgetAllocation(pipeline::device::Device &Device)
{
	...
	struct ShadowValidationState final
	{
		bool SamplingValidated = false;
	};
	ShadowValidationState CallbackOwner;
	const auto NoOpExecutor = [](void *UserData, const graph::HybridDeferredPassID Pass, graph::RenderGraphContext &Context,
								 const graph::HybridDeferredFrameResources &PassResources)
	{
		if (Pass != graph::HybridDeferredPassID::DeferredLighting)
			return;
		for (const graph::TextureHandle Shadow :
			 {PassResources.DirectionalShadowAtlas, PassResources.SpotShadowAtlas, PassResources.PointShadowArray})
		{
			const GLuint Texture = Context.GetTexture(Shadow);
			GLint CompareMode = GL_NONE;
			GLint CompareFunction = GL_NONE;
			GLint MinificationFilter = GL_NONE;
			GLint MagnificationFilter = GL_NONE;
			glGetTextureParameteriv(Texture, GL_TEXTURE_COMPARE_MODE, &CompareMode);
			glGetTextureParameteriv(Texture, GL_TEXTURE_COMPARE_FUNC, &CompareFunction);
			glGetTextureParameteriv(Texture, GL_TEXTURE_MIN_FILTER, &MinificationFilter);
			glGetTextureParameteriv(Texture, GL_TEXTURE_MAG_FILTER, &MagnificationFilter);
			if (CompareMode != GL_COMPARE_REF_TO_TEXTURE || CompareFunction != GL_LEQUAL || MinificationFilter != GL_LINEAR ||
				MagnificationFilter != GL_LINEAR)
				throw std::runtime_error("shadow texture state mismatch: compare mode " + std::to_string(CompareMode) +
										 ", compare function " + std::to_string(CompareFunction) + ", minification " +
										 std::to_string(MinificationFilter) + ", magnification " + std::to_string(MagnificationFilter));
		}
		static_cast<ShadowValidationState *>(UserData)->SamplingValidated = true;
	};
	...
	Graph.Execute();
	Require(CallbackOwner.SamplingValidated, "deferred shadow sampling validation did not execute");
	Device.CheckErrors("maximum bounded shadow-array allocation validation");
}

void ValidateConservativeHierarchicalDepth(pipeline::device::Device &Device)
{
	GLuint SourceDepth = 0;
	GLuint HierarchicalDepth = 0;
	try
	{
		resource::AssetManager Assets;
		pipeline::shader::ShaderLibrary Shaders(Device, Assets);
		const uint32 PipelineID =
			Shaders.CreateComputePipeline({.Compute = {.Path = "Shaders/HiZ.comp", .Stage = pipeline::shader::ShaderStage::Compute}});
		auto &Pipeline = Shaders.GetComputePipeline(PipelineID);

		constexpr std::array<float32, 16> SourceValues = {0.9f, 0.0f, 0.8f, 0.7f, 0.6f, 0.5f, 0.4f, 0.3f,
														  0.2f, 0.1f, 1.0f, 0.9f, 0.8f, 0.7f, 0.6f, 0.5f};
		glCreateTextures(GL_TEXTURE_2D, 1, &SourceDepth);
		glTextureStorage2D(SourceDepth, 1, GL_R32F, 4, 4);
		glTextureSubImage2D(SourceDepth, 0, 0, 0, 4, 4, GL_RED, GL_FLOAT, SourceValues.data());
		glCreateTextures(GL_TEXTURE_2D, 1, &HierarchicalDepth);
		glTextureStorage2D(HierarchicalDepth, 3, GL_R32F, 4, 4);

		glBindTextureUnit(0, SourceDepth);
		glBindImageTexture(0, HierarchicalDepth, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_R32F);
		Pipeline.SetUniformUInt2(pipeline::shader::ComputeUniform::SourceExtent, 4U, 4U);
		Pipeline.SetUniformUInt(pipeline::shader::ComputeUniform::SourceMip, 0U);
		Pipeline.SetUniformUInt(pipeline::shader::ComputeUniform::SourceScale, 1U);
		Pipeline.Bind();
		glDispatchCompute(1, 1, 1);
		glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT | GL_TEXTURE_FETCH_BARRIER_BIT);

		glBindTextureUnit(0, HierarchicalDepth);
		glBindImageTexture(0, HierarchicalDepth, 1, GL_FALSE, 0, GL_WRITE_ONLY, GL_R32F);
		Pipeline.SetUniformUInt2(pipeline::shader::ComputeUniform::SourceExtent, 4U, 4U);
		Pipeline.SetUniformUInt(pipeline::shader::ComputeUniform::SourceMip, 0U);
		Pipeline.SetUniformUInt(pipeline::shader::ComputeUniform::SourceScale, 2U);
		Pipeline.Bind();
		glDispatchCompute(1, 1, 1);
		glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT | GL_TEXTURE_FETCH_BARRIER_BIT | GL_TEXTURE_UPDATE_BARRIER_BIT);

		std::array<float32, 4> Reduced{};
		glGetTextureImage(HierarchicalDepth, 1, GL_RED, GL_FLOAT, static_cast<GLsizei>(sizeof(Reduced)), Reduced.data());
		Device.CheckErrors("conservative hierarchical-depth reduction validation");
		std::ranges::sort(Reduced);
		constexpr std::array<float32, 4> Expected = {0.0f, 0.1f, 0.3f, 0.5f};
		for (usize Index = 0; Index < Reduced.size(); ++Index)
		{
			Require(std::abs(Reduced[Index] - Expected[Index]) < 0.00001f,
					"reversed-Z hierarchical depth did not preserve the farthest depth in every reduction block");
		}
	}
	catch (...)
	{
		if (HierarchicalDepth != 0)
			glDeleteTextures(1, &HierarchicalDepth);
		if (SourceDepth != 0)
			glDeleteTextures(1, &SourceDepth);
		throw;
	}
	if (HierarchicalDepth != 0)
		glDeleteTextures(1, &HierarchicalDepth);
...
void ValidateGPUVisibilityCompaction(pipeline::device::Device &Device)
{
	...
	try
	{
		resource::AssetManager Assets;
		pipeline::shader::ShaderLibrary Shaders(Device, Assets);
		const uint32 CullPipeline = Shaders.CreateComputePipeline(
			{.Compute = {.Path = "Shaders/Visibility.comp", .Stage = pipeline::shader::ShaderStage::Compute}});
		const uint32 PrefixPipeline = Shaders.CreateComputePipeline(
			{.Compute = {.Path = "Shaders/VisibilityPrefixScan.comp", .Stage = pipeline::shader::ShaderStage::Compute}});
		const uint32 BlockPrefixPipeline = Shaders.CreateComputePipeline(
			{.Compute = {.Path = "Shaders/VisibilityBlockPrefixScan.comp", .Stage = pipeline::shader::ShaderStage::Compute}});
		const uint32 FinalizePipeline = Shaders.CreateComputePipeline(
			{.Compute = {.Path = "Shaders/VisibilityFinalize.comp", .Stage = pipeline::shader::ShaderStage::Compute}});
		const uint32 ScatterPipeline = Shaders.CreateComputePipeline(
			{.Compute = {.Path = "Shaders/VisibilityScatter.comp", .Stage = pipeline::shader::ShaderStage::Compute}});
...
void ValidateShaderResourceContracts(pipeline::device::Device &Device)
{
	...
	const uint32 ValidPipeline =
		Shaders.CreateComputePipeline({.Compute = {.Path = "Shaders/Visibility.comp", .Stage = pipeline::shader::ShaderStage::Compute},
	...
	const uint32 TemporalPipelineID = Shaders.CreateComputePipeline(
		{.Compute = {.Path = "Shaders/TAA.comp", .Stage = pipeline::shader::ShaderStage::Compute},
		 ...
		 .RequiredResources = {
			 ...
			 {.Name = "nextObjectIDHistory",
			  ...
			  .Binding = 2}}});
	std::array<GLuint, 8> TemporalTextures{};
	glCreateTextures(GL_TEXTURE_2D, static_cast<GLsizei>(TemporalTextures.size()), TemporalTextures.data());
	struct TemporalTextureCleanup final
	{
		std::array<GLuint, 8> &Textures;
		~TemporalTextureCleanup()
		{
			glDeleteTextures(static_cast<GLsizei>(this->Textures.size()), this->Textures.data());
		}
	};
	[[maybe_unused]] TemporalTextureCleanup TextureCleanup{TemporalTextures};
	for (uint32 Index : {0U, 1U, 5U, 6U})
		glTextureStorage2D(TemporalTextures[Index], 1, GL_RGBA16F, 8, 8);
	glTextureStorage2D(TemporalTextures[2], 1, GL_RG16F, 8, 8);
	for (uint32 Index : {3U, 4U, 7U})
		glTextureStorage2D(TemporalTextures[Index], 1, GL_R32UI, 8, 8);
	for (GLuint Texture : TemporalTextures)
	{
	...
	std::array<glm::vec4, 64> CurrentSamples{};
	std::array<glm::vec4, 64> HistorySamples{};
	for (uint32 Y = 0; Y < 8U; ++Y)
	{
		for (uint32 X = 0; X < 8U; ++X)
		{
			const usize Index = static_cast<usize>(Y) * 8U + X;
			CurrentSamples[Index] = X < 4U	  ? glm::vec4(0.5f, 0.5f, 0.5f, 1.0f)
									: X == 4U ? glm::vec4(0.25f, 0.25f, 0.25f, 0.45f)
											  : glm::vec4(0.05f, 0.05f, 0.05f, 0.0f);
			HistorySamples[Index] = CurrentSamples[Index];
			if (X == 4U)
				HistorySamples[Index] = glm::vec4(0.27f, 0.27f, 0.27f, 0.50f);
			if (X == 5U)
				HistorySamples[Index] = glm::vec4(0.50f, 0.50f, 0.50f, 1.0f);
			if (X == 6U)
				HistorySamples[Index] = glm::vec4(0.10f, 0.10f, 0.10f, 0.0f);
		}
	}
	glTextureSubImage2D(TemporalTextures[0], 0, 0, 0, 8, 8, GL_RGBA, GL_FLOAT, CurrentSamples.data());
	glTextureSubImage2D(TemporalTextures[1], 0, 0, 0, 8, 8, GL_RGBA, GL_FLOAT, HistorySamples.data());
	const glm::vec2 ZeroVelocity(0.0f);
	glClearTexImage(TemporalTextures[2], 0, GL_RG, GL_FLOAT, &ZeroVelocity[0]);
	const uint32 ObjectID = 1U;
	glClearTexImage(TemporalTextures[3], 0, GL_RED_INTEGER, GL_UNSIGNED_INT, &ObjectID);
	glClearTexImage(TemporalTextures[4], 0, GL_RED_INTEGER, GL_UNSIGNED_INT, &ObjectID);
	for (uint32 Unit = 0; Unit < 5U; ++Unit)
		glBindTextureUnit(Unit, TemporalTextures[Unit]);
	glBindImageTexture(0, TemporalTextures[5], 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA16F);
	glBindImageTexture(1, TemporalTextures[6], 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA16F);
	glBindImageTexture(2, TemporalTextures[7], 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_R32UI);
	auto &TemporalPipeline = Shaders.GetComputePipeline(TemporalPipelineID);
	TemporalPipeline.SetUniformUInt(pipeline::shader::ComputeUniform::HistoryValid, 1U);
	TemporalPipeline.Bind();
	glDispatchCompute(1, 1, 1);
	glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT | GL_TEXTURE_FETCH_BARRIER_BIT);
	std::array<glm::vec4, 64> ResolvedSamples{};
	glGetTextureImage(TemporalTextures[6], 0, GL_RGBA, GL_FLOAT, static_cast<GLsizei>(sizeof(ResolvedSamples)), ResolvedSamples.data());
	const glm::vec4 ResolvedEdge = ResolvedSamples[4U + 4U * 8U];
	Require(ResolvedEdge.r > 0.25f && ResolvedEdge.r < 0.27f, "TAA did not accumulate stable sub-pixel coverage at a shadow edge");
	const glm::vec4 ResolvedDisplacedShadow = ResolvedSamples[5U + 4U * 8U];
	Require(glm::all(glm::lessThan(glm::abs(glm::vec3(ResolvedDisplacedShadow) - glm::vec3(0.05f)), glm::vec3(0.005f))),
			"TAA accumulated a displaced shadow edge");
	const glm::vec4 ResolvedShadowInterior = ResolvedSamples[6U + 4U * 8U];
	Require(glm::all(glm::lessThan(glm::abs(glm::vec3(ResolvedShadowInterior) - glm::vec3(0.05f)), glm::vec3(0.005f))),
			"TAA blurred the interior of a direct shadow");
	Device.CheckErrors("temporal shadow rejection validation");
}
...
void ValidateReservedAssetLoading()
{
	...
	const std::filesystem::path RootPath = "Shaders/Fullscreen.vert";
	const std::filesystem::path ReservedPath = "Shaders/ToneMap.frag";
...
void ValidatePrivateMaterialExtraction(pipeline::device::Device &Device)
{
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameMaterialExtractionValidation-" + util::UUID::GenerateRandomUUID().ToString());
	...
	pipeline::render::RenderLODHistory PreviousLODs;
	pipeline::render::RenderLODHistory CurrentLODs;
	pipeline::render::SceneExtractorScratch Scratch;
	pipeline::render::SceneExtractor Extractor(Device, MeshCache, Assets, PreviousTransforms, 0, CurrentTransforms, PreviousLODs,
											   CurrentLODs, 1, Scratch);
...
void ValidatePrimitiveLODStability(pipeline::device::Device &Device)
{
	resource::AssetManager Assets;
	editor::asset::PrimitiveMeshFactory Factory(Assets);
	const resource::AssetHandle<resource::ModelAsset> Model = Factory.GetModel(editor::asset::PrimitiveShape::Cone);
	const resource::AssetPtr<resource::ModelAsset> PinnedModel = Model.Pin();
	Require(PinnedModel != nullptr && !PinnedModel->GetMeshInstances().empty(), "validation cone model is unavailable");
	const resource::AssetPtr<resource::MeshAsset> Mesh = PinnedModel->GetMeshInstances().front().Mesh.Pin();
	Require(Mesh != nullptr && Mesh->GetLODs().size() >= 3U, "validation cone does not expose its expected LOD chain");

	pipeline::mesh::MeshGPUCache MeshCache(Device);
	pipeline::mesh::MeshGPUResource &GPUResource = MeshCache.Realize(Mesh, 1);
	const uint32 ResidentLOD1VertexArray = GPUResource.GetLOD(1, 1).VertexArray;
	const pipeline::mesh::MeshGPULOD *Fallback = MeshCache.TryGetLOD(Mesh, 2, 2);
	Require(Fallback != nullptr && Fallback->Level == 1U && Fallback->VertexArray == ResidentLOD1VertexArray,
			"a transitioning cone disappeared instead of using its nearest resident LOD");
	(void)MeshCache.ProcessPending(8, std::chrono::seconds(1), 2);
	for (uint32 Level = 0; Level < Mesh->GetLODs().size(); ++Level)
		(void)GPUResource.GetLOD(Level, 2);

	editor::document::SceneDocument Document("Primitive LOD stability");
	const world::ObjectHandle Object = Document.CreateObject("ValidationCone");
	(void)Document.GetScene().AddComponent<components::CObjectMeshComponent>(Object, Model);
	{
		auto Access = Document.GetScene().Write();
		Access.Resolve(Access.GetComponent<components::CObjectMeshComponent>(Object))
			.SetVisibility(components::MeshVisibilityFlags::Visible | components::MeshVisibilityFlags::ReceivesShadows);
	}
	const pipeline::render::SceneRenderSnapshot Snapshot = pipeline::render::SceneRenderSnapshotBuilder::Build(Document.GetScene());
	pipeline::render::RenderTransformHistory PreviousTransforms;
	pipeline::render::RenderTransformHistory CurrentTransforms;
	...
	pipeline::render::RenderLODHistory CurrentLODs;
	pipeline::render::SceneExtractorScratch Scratch;
	Camera ViewCamera(0.1f, 45.0f, 0.1f, 1'000.0f);
	const glm::vec4 Sphere = Mesh->GetBounds().Sphere;
	const float32 TanHalfFOV = std::tan(glm::radians(ViewCamera.FOV) * 0.5f);
	uint64 PreviousGeneration = 0;
	uint64 CurrentGeneration = 1;
	const auto ExtractVertexArray = [&](const pipeline::render::SceneRenderSnapshot &FrameSnapshot, const float32 Coverage)
	{
		const float32 SurfaceDistance = Sphere.w / (Coverage * TanHalfFOV);
		ViewCamera.Position = glm::vec3(Sphere) + glm::vec3(0.0f, 0.0f, Sphere.w + SurfaceDistance);
		pipeline::render::SceneExtractor Extractor(Device, MeshCache, Assets, PreviousTransforms, PreviousGeneration, CurrentTransforms,
												   PreviousLODs, CurrentLODs, CurrentGeneration, Scratch);
		SceneCollection Collection;
		Collection.BeginFrame(CurrentGeneration);
		Extractor.Extract(FrameSnapshot, ViewCamera, Collection);
		Collection.Seal();
		Require(!Collection.GetRenderItems().empty(), "camera motion dropped the validation cone during LOD selection");
		const uint32 VertexArray = Collection.GetRenderItems().front().VertexArray;
		PreviousTransforms.swap(CurrentTransforms);
		PreviousLODs.swap(CurrentLODs);
		PreviousGeneration = CurrentGeneration;
		++CurrentGeneration;
		return VertexArray;
	};

	const uint32 LOD1VertexArray = GPUResource.GetLOD(1, 2).VertexArray;
	const uint32 LOD2VertexArray = GPUResource.GetLOD(2, 2).VertexArray;
	Require(ExtractVertexArray(Snapshot, 0.36f) == LOD1VertexArray, "validation cone did not select LOD 1 above its transition");
	Require(ExtractVertexArray(Snapshot, 0.34f) == LOD1VertexArray, "cone LOD oscillated inside its configured hysteresis band");
	Require(ExtractVertexArray(Snapshot, 0.32f) == LOD2VertexArray, "cone LOD did not transition after leaving its hysteresis band");
	{
		auto Access = Document.GetScene().Write();
		Access.Resolve(Access.GetComponent<components::CObjectMeshComponent>(Object))
			.SetVisibility(components::MeshVisibilityFlags::Visible | components::MeshVisibilityFlags::CastsShadows |
						   components::MeshVisibilityFlags::ReceivesShadows);
	}
	const pipeline::render::SceneRenderSnapshot ShadowSnapshot = pipeline::render::SceneRenderSnapshotBuilder::Build(Document.GetScene());
	Require(ExtractVertexArray(ShadowSnapshot, 0.1f) == GPUResource.GetLOD(0, 2).VertexArray,
			"curved primitive shadow caster did not retain its source silhouette LOD");
	MeshCache.Clear();
}
} // namespace
...
void RunDeterministicRenderCoreChecks(pipeline::device::Device &Device)
{
	...
	std::cerr << "[Validation] Primitive LOD stability\n";
	ValidatePrimitiveLODStability(Device);
	...
	std::cerr << "[Validation] Conservative hierarchical depth\n";
	ValidateConservativeHierarchicalDepth(Device);
