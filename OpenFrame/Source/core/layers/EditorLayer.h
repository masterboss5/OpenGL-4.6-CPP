...
#include "Source/editor/ui/EditorUserInterface.h"
...
{
	std::optional<project::ProjectDescriptor> InitialProject;
	std::filesystem::path EngineContentRoot;
	pipeline::render::RenderViewID View{.Value = 2};
	uint64 MaximumRenderedFrames = 0;
	struct ProjectConfiguration final
	{
		std::vector<cook::RuntimePackageFile> RuntimePackageFiles;
	};
	std::function<ProjectConfiguration(const project::ProjectDescriptor &)> ConfigureProject;
};
...
class EditorLayer final : public ApplicationLayer
{
  ...
  private:
	...
	project::ProjectHub ProjectHub;
	EditorLayerSpecification Specification;
