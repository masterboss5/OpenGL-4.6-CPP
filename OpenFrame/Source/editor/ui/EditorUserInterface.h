...
{
class ProjectHub;
struct NewProjectSpecification;
} // namespace project
namespace action
{
...
struct EditorUIFrame final
{
	...
	std::optional<std::filesystem::path> OpenProjectRequest;
	std::optional<std::pair<string, std::filesystem::path>> CreateProjectRequest;
	bool CloseProjectRequest = false;
};
...
class EditorUserInterface final
{
  public:
	...
	[[nodiscard]] EditorUIFrame BuildHomeFrame(const core::ApplicationFrame &Frame, project::ProjectHub &Projects,
											   core::diagnostics::DiagnosticSink &Diagnostics, string_view ProjectDiagnostic,
											   bool AllowWindowMutation);
