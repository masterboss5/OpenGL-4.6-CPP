...
void ValidateSecurePaths()
{
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameSecurePathValidation-" + util::UUID::GenerateRandomUUID().ToString());
...
void ValidateAuthoringContracts()
{
	...
	const project::ProjectDescriptor ManagerDescriptor{
		...
		.DescriptorPath = std::filesystem::temp_directory_path() /
						  ("OpenFrameProjectManagerValidation-" + util::UUID::GenerateRandomUUID().ToString() + ".engineproject")};
...
{
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameAssetRegistryValidation-" + util::UUID::GenerateRandomUUID().ToString());
	std::error_code Error;
...
{
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameAssetImportValidation-" + util::UUID::GenerateRandomUUID().ToString());
...
{
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameAssetContentValidation-" + util::UUID::GenerateRandomUUID().ToString());
	const std::filesystem::path Content = Root / "Content";
	const std::filesystem::path Intermediate = Root / "Intermediate";
...
{
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameMaterialValidation-" + util::UUID::GenerateRandomUUID().ToString());
	const std::filesystem::path Content = Root / "Content";
...
{
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameModelDependencyValidation-" + util::UUID::GenerateRandomUUID().ToString());
	const std::filesystem::path Content = Root / "Content";
...
{
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFramePrivateMaterialValidation-" + util::UUID::GenerateRandomUUID().ToString());
	const std::filesystem::path Content = Root / "Content";
	const std::filesystem::path Intermediate = Root / "Intermediate";
	std::filesystem::create_directories(Content);
	std::filesystem::create_directories(Intermediate);
	...
	const auto RenameFixtureDirectory = [](const std::filesystem::path &Source, const std::filesystem::path &Destination)
	{
		std::error_code Error;
		...
		throw std::filesystem::filesystem_error("private-material failure fixture rename", Source, Destination, Error);
	};
	RenameFixtureDirectory(MaterialDirectory, MaterialDirectoryBackup);
	{
	...
	RenameFixtureDirectory(MaterialDirectoryBackup, MaterialDirectory);
...
{
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameEditorActionValidation-" + util::UUID::GenerateRandomUUID().ToString());
	std::error_code Error;
...
void ValidateEditorSerialization()
{
	...
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameSerializationValidation-" + util::UUID::GenerateRandomUUID().ToString());
...
{
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameEditorLayoutValidation-" + util::UUID::GenerateRandomUUID().ToString());
	struct Cleanup final
	{
		std::filesystem::path Root;
		~Cleanup()
...
void ValidateProjectHub()
{
	Require(project::ProjectHub::GetDefaultProjectsRoot().filename() == "OpenFrame Projects",
			"default project root does not use the required Documents/OpenFrame Projects folder");
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameProjectHub-" + util::UUID::GenerateRandomUUID().ToString());
	struct Cleanup final
	{
		std::filesystem::path Root;
		~Cleanup()
	...
	[[maybe_unused]] Cleanup CleanupScope{Root};
	project::ProjectHub Hub(Root / "Projects", Root / "State");
	const project::ProjectDescriptor Descriptor = Hub.CreateBaseplateProject({.Name = "Project Hub Validation", .ParentDirectory = {}});
	const std::filesystem::path ProjectRoot = Root / "Projects" / "Project Hub Validation";
	Require(Descriptor.Name == "Project Hub Validation" && Descriptor.GameModule.empty() &&
				Descriptor.StartupScene == "Scenes/Baseplate.enginelevel" &&
				std::filesystem::is_regular_file(ProjectRoot / "Project Hub Validation.engineproject") &&
				std::filesystem::is_regular_file(ProjectRoot / "Content" / "Scenes" / "Baseplate.enginelevel") &&
				std::filesystem::is_regular_file(ProjectRoot / "Content" / "Meshes" / "Baseplate.obj"),
			"Baseplate project creation did not publish its descriptor, scene, and plane mesh");
	Require(!std::filesystem::exists(ProjectRoot / "Saved") && !std::filesystem::exists(ProjectRoot / "Intermediate") &&
				!std::filesystem::exists(ProjectRoot / "Build"),
			"Baseplate project creation polluted the authored project with generated directories");
...
{
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameContentBrowserValidation-" + util::UUID::GenerateRandomUUID().ToString());
	struct Cleanup final
	{
		std::filesystem::path Root;
		~Cleanup()
...
{
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameCookValidation-" + util::UUID::GenerateRandomUUID().ToString());
	struct Cleanup final
	{
		std::filesystem::path Root;
		~Cleanup()
	...
	std::filesystem::create_directories(Root / "EngineSource" / "Shaders");
	...
	serialization::ProjectDescriptorSerializer::Save(Descriptor);
	{
		...
		std::ofstream Shader(Root / "EngineSource" / "Shaders" / "Runtime.glsl", std::ios::binary);
		Shader << "#version 460 core\n";
	}
	...
	Service.Begin(Project,
				  {.OutputDirectory = Output,
				   .RuntimeFiles = {{.Source = Root / "RuntimeSource" / "Game.exe",
									...
									{.Source = Root / "EngineSource" / "Shaders" / "Runtime.glsl",
									 .Destination = "Engine/Shaders/Runtime.glsl",
	...
	Require(Mount.ProjectID == Descriptor.ID && Mount.ProjectName == Descriptor.Name &&
				...
				std::filesystem::is_directory(Mount.EngineContentRoot / "Shaders"),
...
{
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameEditorRecoveryValidation-" + util::UUID::GenerateRandomUUID().ToString());
	struct Cleanup final
	{
		std::filesystem::path Root;
		~Cleanup()
...
{
	const std::filesystem::path Root =
		std::filesystem::temp_directory_path() / ("OpenFrameGameModuleValidation-" + util::UUID::GenerateRandomUUID().ToString());
...
{
	const std::filesystem::path Root = std::filesystem::temp_directory_path() / "OpenFrame" / "Validation" /
									   ("ProjectBuild-" + util::UUID::GenerateRandomUUID().ToString());
	struct Cleanup final
	{
		std::filesystem::path Root;
		~Cleanup()
	...
	Service.Configure({.MSBuildExecutable = MSBuild,
					   ...
					   .EngineContentRoot = Solution.parent_path() / "OpenFrame"});
	core::threading::TaskScheduler Scheduler({.WorkerCount = 2, .Capacity = 32});
	...
	Require(ProjectResult.has_value() && ProjectResult->State == build::ProjectBuildState::Succeeded &&
				std::filesystem::is_regular_file(ProjectResult->RuntimeDirectory / "OpenFrameEditor.exe") &&
				std::filesystem::is_regular_file(ProjectResult->RuntimeDirectory / "OpenFrameTools.lib") &&
				std::filesystem::is_regular_file(ProjectResult->RuntimeDirectory / "OpenFrameGame.exe") &&
				std::filesystem::is_regular_file(ProjectResult->RuntimeDirectory / "OpenFrameValidation.exe") &&
				std::filesystem::is_regular_file(ProjectResult->RuntimeDirectory / "OpenFrameEngine.dll"),
