...
class PackageMountLock final
{
  public:
	...
	{
		const string NarrowName = "Local\\OpenFramePackage-" + ProjectID.ToString() + "-" + OperationID.ToString();
...
[[nodiscard]] std::filesystem::path GetCacheRoot()
{
	...
	core::io::SecurePath::CreateDirectoriesWithin(LocalRoot, std::filesystem::path("OpenFrame") / "RuntimeCache",
												  "Runtime package cache root");
	const std::filesystem::path Root = LocalRoot / "OpenFrame" / "RuntimeCache";
	return Root;
}
										  ...
										  const ProjectPackageTrustPolicy &TrustPolicy)
{
	...
	if (!std::filesystem::is_directory(Mount.EngineContentRoot / "Shaders"))
